...
fb = Label(root, text='FACEBOOK', fg='black', font=20)
...
def submit():
    ...
    c.execute('''insert into user values(:firstName, :lastName,:age,:address,:city,:Zipcode,:password,:gender)''',
              {'firstName': firstname.get(), 'lastName': lastname.get(), 'age': age.get(), 'address': address.get(),
               'city': city.get(), 'Zipcode': zipcode.get(), 'password': password.get(), 'gender': gender.get()}
              )
